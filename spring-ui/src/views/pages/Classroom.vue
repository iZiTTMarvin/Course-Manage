...
import {FindClassroomParams} from "@/api/classroom";
...
const tableType  = [
  ...
  , {label: '教室', width: '', rules: [{required: false, message: '请输入教室', trigger: 'blur'}], showType: '', prop: 'room'}
  , {label: '座位', width: '', rules: [{required: false, message: '请输入座位', trigger: 'blur'}], showType: '', prop: 'site'}
...
const SearchForm = ref(<FindClassroomParams>{
  "id": 0,
  "name": "",
  "room": "",
  "site": 0,
...
function getTableData(val: number) {
  api.classroom.listPageClassroom(val, 12).then((res: any) => {
    if (res.success) {
...
function showRecover() {
  ...
  api.classroom.recoverListClassroom().then((res: any) => {
...
function onRecover(index, row) {
  ElMessageBox.confirm('确认恢复此数据吗?', '提示', {
    ...
    api.classroom.recoverClassroom(row.id).then((res: any) => {
      ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
      api.classroom.recoverListClassroom().then((res: any) => {
        RecoverData.value = res.data;
      })
...
function onDeleteForever(index, row) {
  ElMessageBox.confirm('确认删除此数据吗?', '警告', {
    ...
    api.classroom.deleteClassroom(row.id).then((res: any) => {
      ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
      api.classroom.recoverListClassroom().then((res: any) => {
        RecoverData.value = res.data;
      })
...
function onSearch() {
  api.classroom.findClassroom(SearchForm.value).then((res: any) => {
    if (res.success) {
...
function onAppend(formEl: FormInstance | undefined) {
  ...
  formEl.validate((valid) => {
    if (valid) {
      api.classroom.insertClassroom(AppendForm.value).then((res: any) => {
        ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
        if (res.success) {
    ...
    type: 'warning'
      ...
      .then(() => {
        for (let i = 0; i < tableSelection.value.length; i++) {
          ...
          api.classroom.deleteTempClassroom(element.id).then((res) => {
            ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
          })
        }
        getTableData(1);
...
function onImport() {
  api.classroom.importExcelClassroom(ImportForm.value).then((res: any) => {
    ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
  })
}

function onMultiExport() {
  api.classroom.exportExcelClassroom().then((res: any) => {
...
function onEditor(formEl: FormInstance | undefined) {
  ...
  formEl.validate((valid) => {
    if (valid) {
      api.classroom.updateClassroom(EditorForm.value).then((res: any) => {
        ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
        if (res.success) {
...
function onDelete(index: string, row: any) {
  ElMessageBox.confirm('确认删除吗?', '警告', {confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning'}, appContext)
      .then(() => {
        api.classroom.deleteTempClassroom(row.id).then((res: any) => {
          ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
          getTableData(1);
        })
      })
