...
import api from "@/api";
import {FindProfessParams} from "@/api/profess";

const {appContext} = getCurrentInstance()!

const tableType  = [
  {label: '名称', width: '', rules: [{required: 'true', message: '请输入名称', trigger: 'blur'}], showType: '', prop: 'name'}
  , {label: '所属学院@', width: '', rules: [{required: false, message: '请输入所属学院@', trigger: 'blur'}], showType: '', prop: 'aname'}
...
const AppendType = [
  {label: '名称', width: '', rules: [{required: 'true', message: '请输入名称', trigger: 'blur'}], showType: '', prop: 'name'}
  , {label: '所属学院@', width: '', rules: [{required: false, message: '请输入所属学院@', trigger: 'blur'}], showType: '', prop: 'academic'}
  , {label: '状态', width: '', rules: [{required: 'true', message: '请输入状态', trigger: 'blur'}], showType: 'select', prop: 'state'}
];
...
const EditorType = [
  {label: '名称', width: '', rules: [{required: 'true', message: '请输入名称', trigger: 'blur'}], showType: '', prop: 'name'}
  , {label: '所属学院@', width: '', rules: [{required: false, message: '请输入所属学院@', trigger: 'blur'}], showType: '', prop: 'tableList'}
  , {label: '状态', width: '', rules: [{required: 'true', message: '请输入状态', trigger: 'blur'}], showType: 'select', prop: 'state'}
];

const SearchVisible = ref(false);
const SearchForm = ref(<FindProfessParams>{
  "id": 0,
  "name": "",
  "list": "",
  "createBy": "",
...
function getTableData(val: number) {
  api.profess.listPageProfess(val, 12).then((res: any) => {
    if (res.success) {
...
function showRecover() {
  ...
  api.profess.recoverListProfess().then((res: any) => {
...
function onRecover(index, row) {
  ElMessageBox.confirm('确认恢复此数据吗?', '提示', {
    ...
    api.profess.recoverProfess(row.id).then((res: any) => {
      ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
      api.profess.recoverListProfess().then((res: any) => {
        RecoverData.value = res.data;
      })
...
function onDeleteForever(index, row) {
  ElMessageBox.confirm('确认删除此数据吗?', '警告', {
    ...
    api.profess.deleteProfess(row.id).then((res: any) => {
      ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
      api.profess.recoverListProfess().then((res: any) => {
        RecoverData.value = res.data;
      })
...
function onSearch() {
  api.profess.findProfess(SearchForm.value).then((res: any) => {
    if (res.success) {
...
function onAppend(formEl: FormInstance | undefined) {
  ...
  formEl.validate((valid) => {
    if (valid) {
      api.profess.insertProfess(AppendForm.value).then((res: any) => {
        ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
        if (res.success) {
    ...
    type: 'warning'
      ...
      .then(() => {
        for (let i = 0; i < tableSelection.value.length; i++) {
          ...
          api.profess.deleteTempProfess(element.id).then((res) => {
            ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
          })
        }
        getTableData(1);
...
function onImport() {
  api.profess.importExcelProfess(ImportForm.value).then((res: any) => {
    ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
  })
}

function onMultiExport() {
  api.profess.exportExcelProfess().then((res: any) => {
...
function onEditor(formEl: FormInstance | undefined) {
  ...
  formEl.validate((valid) => {
    if (valid) {
      api.profess.updateProfess(EditorForm.value).then((res: any) => {
        ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
        if (res.success) {
...
function onDelete(index: string, row: any) {
  ElMessageBox.confirm('确认删除吗?', '警告', {confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning'}, appContext)
      .then(() => {
        api.profess.deleteTempProfess(row.id).then((res: any) => {
          ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
          getTableData(1);
        })
      })
