...
import api from "@/api";
import {FindListParams} from "@/api/list";

const {appContext} = getCurrentInstance()!

const tableType  = [
  {label: '名称', width: '', rules: [{required: 'true', message: '请输入名称', trigger: 'blur'}], showType: '', prop: 'name'}
  , {label: '学分', width: '', rules: [{required: false, message: '请输入学分', trigger: 'blur'}], showType: '', prop: 'grade'}
  , {label: '类型@', width: '', rules: [{required: false, message: '请输入类型@', trigger: 'blur'}], showType: '', prop: 'tname'}
  , {label: '所属专业@', width: '', rules: [{required: false, message: '请输入所属专业@', trigger: 'blur'}], showType: '', prop: 'pname'}
...
const SearchType = [
  {label: '名称', width: '', rules: [{required: 'true', message: '请输入名称', trigger: 'blur'}], showType: '', prop: 'name'}
  , {label: '学分', width: '', rules: [{required: false, message: '请输入学分', trigger: 'blur'}], showType: '', prop: 'grade'}
  , {label: '类型@', width: '', rules: [{required: false, message: '请输入类型@', trigger: 'blur'}], showType: '', prop: 'type'}
  // , {label: '所属专业@', width: '', rules: [{required: false, message: '请输入所属专业@', trigger: 'blur'}], showType: '', prop: 'profess'}
  , {label: '状态', width: '', rules: [{required: 'true', message: '请输入状态', trigger: 'blur'}], showType: 'select', prop: 'state'}
];
...
const SearchForm = ref(<FindListParams>{
  "id": 0,
  "name": "",
  "grade": "",
  "type": 0,
...
function getTableData(val: number) {
  api.list.listPageList(val, 12).then((res: any) => {
    if (res.success) {
...
function showRecover() {
  ...
  api.list.recoverListList().then((res: any) => {
...
function onRecover(index, row) {
  ElMessageBox.confirm('确认恢复此数据吗?', '提示', {
    ...
    api.list.recoverList(row.id).then((res: any) => {
      ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
      api.list.recoverListList().then((res: any) => {
        RecoverData.value = res.data;
      })
...
function onDeleteForever(index, row) {
  ElMessageBox.confirm('确认删除此数据吗?', '警告', {
    ...
    api.list.deleteList(row.id).then((res: any) => {
      ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
      api.list.recoverListList().then((res: any) => {
        RecoverData.value = res.data;
      })
...
function onSearch() {
  api.list.findList(SearchForm.value).then((res: any) => {
    if (res.success) {
...
function onAppend(formEl: FormInstance | undefined) {
  ...
  formEl.validate((valid) => {
    if (valid) {
      api.list.insertList(AppendForm.value).then((res: any) => {
        ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
        if (res.success) {
    ...
    type: 'warning'
      ...
      .then(() => {
        for (let i = 0; i < tableSelection.value.length; i++) {
          ...
          api.list.deleteTempList(element.id).then((res) => {
            ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
          })
        }
        getTableData(1);
...
function onImport() {
  api.list.importExcelList(ImportForm.value).then((res: any) => {
    ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
  })
}

function onMultiExport() {
  api.list.exportExcelList().then((res: any) => {
...
function onEditor(formEl: FormInstance | undefined) {
  ...
  formEl.validate((valid) => {
    if (valid) {
      api.list.updateList(EditorForm.value).then((res: any) => {
        ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
        if (res.success) {
...
function onDelete(index: string, row: any) {
  ElMessageBox.confirm('确认删除吗?', '警告', {confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning'}, appContext)
      .then(() => {
        api.list.deleteTempList(row.id).then((res: any) => {
          ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
          getTableData(1);
        })
      })
