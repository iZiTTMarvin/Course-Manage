...
import api from "@/api";
import {FindTimeParams} from "@/api/time";

const {appContext} = getCurrentInstance()!

const tableType  = [
  {label: '名称', width: '', rules: [{required: 'true', message: '请输入名称', trigger: 'blur'}], showType: '', prop: 'name'}
  , {label: '开始时间', width: '', rules: [{required: false, message: '请输入开始时间', trigger: 'blur'}], showType: '', prop: 'start'}
  , {label: '结束时间', width: '', rules: [{required: false, message: '请输入结束时间', trigger: 'blur'}], showType: '', prop: 'end'}
...
const SearchForm = ref(<FindTimeParams>{
  "id": 0,
  "name": "",
  "start": "",
  "end": "",
...
function getTableData(val: number) {
  api.time.listPageTime(val, 12).then((res: any) => {
    if (res.success) {
...
function showRecover() {
  ...
  api.time.recoverListTime().then((res: any) => {
...
function onRecover(index, row) {
  ElMessageBox.confirm('确认恢复此数据吗?', '提示', {
    ...
    api.time.recoverTime(row.id).then((res: any) => {
      ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
      api.time.recoverListTime().then((res: any) => {
        RecoverData.value = res.data;
      })
...
function onDeleteForever(index, row) {
  ElMessageBox.confirm('确认删除此数据吗?', '警告', {
    ...
    api.time.deleteTime(row.id).then((res: any) => {
      ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
      api.time.recoverListTime().then((res: any) => {
        RecoverData.value = res.data;
      })
...
function onSearch() {
  api.time.findTime(SearchForm.value).then((res: any) => {
    if (res.success) {
...
function onAppend(formEl: FormInstance | undefined) {
  ...
  formEl.validate((valid) => {
    if (valid) {
      api.time.insertTime(AppendForm.value).then((res: any) => {
        ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
        if (res.success) {
    ...
    type: 'warning'
      ...
      .then(() => {
        for (let i = 0; i < tableSelection.value.length; i++) {
          ...
          api.time.deleteTempTime(element.id).then((res) => {
            ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
          })
        }
        getTableData(1);
...
function onImport() {
  api.time.importExcelTime(ImportForm.value).then((res: any) => {
    ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
  })
}

function onMultiExport() {
  api.time.exportExcelTime().then((res: any) => {
...
function onEditor(formEl: FormInstance | undefined) {
  ...
  formEl.validate((valid) => {
    if (valid) {
      api.time.updateTime(EditorForm.value).then((res: any) => {
        ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
        if (res.success) {
...
function onDelete(index: string, row: any) {
  ElMessageBox.confirm('确认删除吗?', '警告', {confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning'}, appContext)
      .then(() => {
        api.time.deleteTempTime(row.id).then((res: any) => {
          ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
          getTableData(1);
        })
      })
