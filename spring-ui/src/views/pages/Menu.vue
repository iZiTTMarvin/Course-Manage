...
import api from "@/api";
import {FindMenuParams} from "@/api/menu";
...
const tableType  = [
  ...
  , {label: '图标', width: '', rules: [{required: false, message: '请输入图标', trigger: 'blur'}], showType: 'icon', prop: 'icon'}
  , {label: '标识符', width: '', rules: [{required: false, message: '请输入标识符', trigger: 'blur'}], showType: '', prop: 'keypath'}
  , {label: '排序', width: '', rules: [{required: false, message: '请输入排序', trigger: 'blur'}], showType: '', prop: 'ranks'}
...
const SearchForm = ref(<FindMenuParams>{
  "id": 0,
  "name": "",
  "icon": "",
  "keypath": "",
...
function getTableData(val: number) {
  api.menu.listPageMenu(val, 12).then((res: any) => {
    if (res.success) {
...
function showRecover() {
  ...
  api.menu.recoverListMenu().then((res: any) => {
...
function onRecover(index, row) {
  ElMessageBox.confirm('确认恢复此数据吗?', '提示', {
    ...
    api.menu.recoverMenu(row.id).then((res: any) => {
      ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
      api.menu.recoverListMenu().then((res: any) => {
        RecoverData.value = res.data;
      })
...
function onDeleteForever(index, row) {
  ElMessageBox.confirm('确认删除此数据吗?', '警告', {
    ...
    api.menu.deleteMenu(row.id).then((res: any) => {
      ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
      api.menu.recoverListMenu().then((res: any) => {
        RecoverData.value = res.data;
      })
...
function onSearch() {
  api.menu.findMenu(SearchForm.value).then((res: any) => {
    if (res.success) {
...
function onAppend(formEl: FormInstance | undefined) {
  ...
  formEl.validate((valid) => {
    if (valid) {
      api.menu.insertMenu(AppendForm.value).then((res: any) => {
        ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
        if (res.success) {
    ...
    type: 'warning'
      ...
      .then(() => {
        for (let i = 0; i < tableSelection.value.length; i++) {
          ...
          api.menu.deleteTempMenu(element.id).then((res) => {
            ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
          })
        }
        getTableData(1);
...
function onImport() {
  api.menu.importExcelMenu(ImportForm.value).then((res: any) => {
    ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
  })
}
...
function onEditor(formEl: FormInstance | undefined) {
  ...
  formEl.validate((valid) => {
    if (valid) {
      api.menu.updateMenu(EditorForm.value).then((res: any) => {
        ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
        if (res.success) {
...
function onDelete(index: string, row: any) {
  ElMessageBox.confirm('确认删除吗?', '警告', {confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning'}, appContext)
      .then(() => {
        api.menu.deleteTempMenu(row.id).then((res: any) => {
          ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
          getTableData(1);
        })
      })
