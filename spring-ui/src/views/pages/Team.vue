...
import api from "@/api";
import {FindTeamParams} from "@/api/team";
...
const SearchForm = ref(<FindTeamParams>{
  "id": 0,
  "name": "",
  "profess": "",
  "createBy": "",
...
function getTableData(val: number) {
  api.team.listPageTeam(val, 12).then((res: any) => {
    if (res.success) {
...
function showRecover() {
  ...
  api.team.recoverListTeam().then((res: any) => {
...
function onRecover(index, row) {
  ElMessageBox.confirm('确认恢复此数据吗?', '提示', {
    ...
    api.team.recoverTeam(row.id).then((res: any) => {
      ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
      api.team.recoverListTeam().then((res: any) => {
        RecoverData.value = res.data;
      })
...
function onDeleteForever(index, row) {
  ElMessageBox.confirm('确认删除此数据吗?', '警告', {
    ...
    api.team.deleteTeam(row.id).then((res: any) => {
      ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
      api.team.recoverListTeam().then((res: any) => {
        RecoverData.value = res.data;
      })
...
function onSearch() {
  api.team.findTeam(SearchForm.value).then((res: any) => {
    if (res.success) {
...
function onAppend(formEl: FormInstance | undefined) {
  ...
  formEl.validate((valid) => {
    if (valid) {
      api.team.insertTeam(AppendForm.value).then((res: any) => {
        ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
        if (res.success) {
    ...
    type: 'warning'
      ...
      .then(() => {
        for (let i = 0; i < tableSelection.value.length; i++) {
          ...
          api.team.deleteTempTeam(element.id).then((res) => {
            ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
          })
        }
        getTableData(1);
...
function onImport() {
  api.team.importExcelTeam(ImportForm.value).then((res: any) => {
    ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
  })
}

function onMultiExport() {
  api.team.exportExcelTeam().then((res: any) => {
...
function onEditor(formEl: FormInstance | undefined) {
  ...
  formEl.validate((valid) => {
    if (valid) {
      api.team.updateTeam(EditorForm.value).then((res: any) => {
        ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
        if (res.success) {
...
function onDelete(index: string, row: any) {
  ElMessageBox.confirm('确认删除吗?', '警告', {confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning'}, appContext)
      .then(() => {
        api.team.deleteTempTeam(row.id).then((res: any) => {
          ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
          getTableData(1);
        })
      })
