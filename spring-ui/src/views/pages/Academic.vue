...
import api from "@/api";
import {FindAcademicParams} from "@/api/academic";

const {appContext} = getCurrentInstance()!

const tableType = [
  {label: '名称', width: '', rules: [{required: 'true', message: '请输入名称', trigger: 'blur'}], showType: '', prop: 'name'}
  ,{label: '专业数量', width: '', rules: [{required: 'true', message: '请输入名称', trigger: 'blur'}], showType: '', prop: 'count'}
  , {
    label: '状态',
    width: '',
...
const DetailType = [
  ...
  , {
    label: '创建者',
    width: '',
    rules: [{required: false, message: '请输入创建者', trigger: 'blur'}],
    showType: '',
    prop: 'createBy'
  }
  , {
  ...
  , {
    label: '更新者',
    width: '',
    rules: [{required: false, message: '请输入更新者', trigger: 'blur'}],
    showType: '',
    prop: 'updateBy'
  }
  , {
  ...
  , {
    label: '伪删除',
    width: '',
    rules: [{required: false, message: '请输入伪删除', trigger: 'blur'}],
    showType: '',
    prop: 'undock'
  }
  , {
...
const SearchForm = ref(<FindAcademicParams>{
  "id": 0,
  "name": "",
  "createBy": "",
  "createTime": "",
...
function getTableData(val: number) {
  api.academic.listPageAcademic(val, 12).then((res: any) => {
    if (res.success) {
...
function showRecover() {
  ...
  api.academic.recoverListAcademic().then((res: any) => {
...
function onRecover(index, row) {
  ElMessageBox.confirm('确认恢复此数据吗?', '提示', {
    ...
    api.academic.recoverAcademic(row.id).then((res: any) => {
      ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
      api.academic.recoverListAcademic().then((res: any) => {
        RecoverData.value = res.data;
      })
...
function onDeleteForever(index, row) {
  ElMessageBox.confirm('确认删除此数据吗?', '警告', {
    ...
    api.academic.deleteAcademic(row.id).then((res: any) => {
      ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
      api.academic.recoverListAcademic().then((res: any) => {
        RecoverData.value = res.data;
      })
...
function onSearch() {
  api.academic.findAcademic(SearchForm.value).then((res: any) => {
    if (res.success) {
...
function onAppend(formEl: FormInstance | undefined) {
  ...
  formEl.validate((valid) => {
    if (valid) {
      api.academic.insertAcademic(AppendForm.value).then((res: any) => {
        ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
        if (res.success) {
    ...
    type: 'warning'
      ...
      .then(() => {
        for (let i = 0; i < tableSelection.value.length; i++) {
          ...
          api.academic.deleteTempAcademic(element.id).then((res) => {
            ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
          })
        }
        getTableData(1);
...
function onImport() {
  api.academic.importExcelAcademic(ImportForm.value).then((res: any) => {
    ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
  })
}

function onMultiExport() {
  api.academic.exportExcelAcademic().then((res: any) => {
...
function onEditor(formEl: FormInstance | undefined) {
  ...
  formEl.validate((valid) => {
    if (valid) {
      api.academic.updateAcademic(EditorForm.value).then((res: any) => {
        ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
        if (res.success) {
...
function onDelete(index: string, row: any) {
  ElMessageBox.confirm('确认删除吗?', '警告', {confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning'}, appContext)
      .then(() => {
        api.academic.deleteTempAcademic(row.id).then((res: any) => {
          ElMessage({message: res.message, type: res.success ? 'success' : 'error', grouping: true, showClose: true});
          getTableData(1);
        })
      })
